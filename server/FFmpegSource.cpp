...
void FFmpegSource::play(const string &ffmpeg_cmd_key, const string &src_url, const string &dst_url, int timeout_ms, const vector<string> &cmd_format_args, const onPlay &cb) {
    ...
    _timer = std::make_shared<Timer>(1.0f, [weakSelf, timeout_ms]() {
        ...
        if (is_local_ip(strongSelf->_media_info.host)) {
            ...
            strongSelf->findAsync(0, [&](const MediaSource::Ptr &src) {
                ...
                if (!src || needRestart) {
                    ...
                    if (strongSelf->_replay_ticker.elapsedTime() > 20 * 1000) {
                        ...
                        strongSelf->play(strongSelf->_ffmpeg_cmd_key, strongSelf->_src_url, strongSelf->_dst_url,timeout_ms, empty_args, [](const SockException &) {});
                    }
                }
            });
        } else {
            ...
            if (!strongSelf->_process.wait(false) || needRestart) {
                ...
                // ffmpeg不在线，重新拉流
                vector<string> empty_args;
                strongSelf->play(strongSelf->_ffmpeg_cmd_key, strongSelf->_src_url, strongSelf->_dst_url, timeout_ms, empty_args, [weakSelf](const SockException &ex) {
                    if (!ex) {
                        // 没有错误
                        return;
                    }
