...
// clang-format off
	...
	{
		{ RTC::CryptoSuite::AEAD_AES_256_GCM, "SRTP_AEAD_AES_256_GCM" },
		{ RTC::CryptoSuite::AEAD_AES_128_GCM, "SRTP_AEAD_AES_128_GCM" },
		{ RTC::CryptoSuite::AES_CM_128_HMAC_SHA1_80, "SRTP_AES128_CM_SHA1_80" },
		{ RTC::CryptoSuite::AES_CM_128_HMAC_SHA1_32, "SRTP_AES128_CM_SHA1_32" }
	};
// clang-format on
...
void DtlsTransport::ClassDestroy() {
  ...
  if (DtlsTransport::privateKey) EVP_PKEY_free(DtlsTransport::privateKey);
  if (DtlsTransport::certificate) X509_free(DtlsTransport::certificate);
  if (DtlsTransport::sslCtx) SSL_CTX_free(DtlsTransport::sslCtx);
}
...
void DtlsTransport::Run(Role localRole) {
  ...
  MS_ASSERT(localRole == Role::CLIENT || localRole == Role::SERVER,
...
bool DtlsTransport::SetRemoteFingerprint(Fingerprint fingerprint) {
  ...
  MS_ASSERT(fingerprint.algorithm != FingerprintAlgorithm::NONE,
...
void DtlsTransport::SendApplicationData(const uint8_t* data, size_t len) {
  ...
  if (written < 0) {
    ...
    if (!CheckStatus(written)) return;
  } else if (written != static_cast<int>(len)) {
    MS_WARN_TAG(dtls, "OpenSSL SSL_write() wrote less (%d bytes) than given data (%zu bytes)",
                written, len);
  }
...
inline bool DtlsTransport::SetTimeout() {
  ...
  MS_ASSERT(this->state == DtlsState::CONNECTING || this->state == DtlsState::CONNECTED,
            "invalid DTLS state");
...
//            "remote fingerprint not set");
  ...
  RTC::CryptoSuite srtpCryptoSuite = GetNegotiatedSrtpCryptoSuite();

  if (srtpCryptoSuite != RTC::CryptoSuite::NONE) {
...
inline void DtlsTransport::ExtractSrtpKeys(RTC::CryptoSuite srtpCryptoSuite) {
  MS_TRACE();
...
inline RTC::CryptoSuite DtlsTransport::GetNegotiatedSrtpCryptoSuite() {
  MS_TRACE();

  RTC::CryptoSuite negotiatedSrtpCryptoSuite = RTC::CryptoSuite::NONE;
  ...
  if (!sslSrtpCryptoSuite) return negotiatedSrtpCryptoSuite;
  ...
  MS_ASSERT(negotiatedSrtpCryptoSuite != RTC::CryptoSuite::NONE,
