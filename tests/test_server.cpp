...
void initEventListener() {
    static onceToken s_token([]() {
        ...
        // Listen to the kBroadcastOnGetRtspRealm event to determine if rtsp link needs authentication (traditional rtsp authentication scheme) to access
        NoticeCenter::Instance().addListener(nullptr, Broadcast::kBroadcastOnGetRtspRealm, [](BroadcastOnGetRtspRealmArgs) {
             ...
             if (string("1") == args.stream) {
                 ...
                 // This stream needs authentication and sets the realm
                 invoker(REALM);
             } else {
                 // 有时我们要查询redis或数据库来判断该流是否需要认证，通过invoker的方式可以做到完全异步  [AUTO-TRANSLATED:41d2819d]
                 // Sometimes we need to query redis or database to determine if the stream needs authentication, which can be done asynchronously through the invoker
        ...
        NoticeCenter::Instance().addListener(nullptr, Broadcast::kBroadcastOnRtspAuth, [](BroadcastOnRtspAuthArgs) {
            ...
            if (user == "test2" && must_no_encrypt) {
                ...
                // Assuming login is test2, and login in base64 format, at this time we provide encrypted password, which will cause authentication failure
                // 可以通过这个方式屏蔽base64这种不安全的加密方式  [AUTO-TRANSLATED:f303f620]
                // You can shield the insecure encryption method of base64 through this method
                invoker(true, "pwd2");
                return;
            }

            // 其他用户密码跟用户名一致  [AUTO-TRANSLATED:53aa89d9]
            // Other user passwords are the same as the username
            invoker(false, user);
        });


        // 监听rtsp/rtmp推流事件，返回结果告知是否有推流权限  [AUTO-TRANSLATED:6c16b087]
        // Listen to the rtsp/rtmp push event and return the result to inform whether there is push permission
        ...
        // Listen to the rtsp/rtsps/rtmp/http-flv playback event and return the result to inform whether there is playback permission (rtsp can implement authentication through kBroadcastOnRtspAuth or this event)
        ...
        // Shell login event, you can log in to the server through the shell to execute some commands
        ...
        // Listen to the rtsp/rtmp source registration or cancellation event; this is used to test rtmp saving as flv recording, saved in the http root directory
        ...
        // Listen to the playback failure (stream not found) event
        NoticeCenter::Instance().addListener(nullptr, Broadcast::kBroadcastNotFoundStream, [](BroadcastNotFoundStreamArgs) {
            /**
             * 你可以在这个事件触发时再去拉流，这样就可以实现按需拉流
             * 拉流成功后，ZLMediaKit会把其立即转发给播放器(最大等待时间约为5秒，如果5秒都未拉流成功，播放器会播放失败)
             * You can pull the stream again when this event is triggered, so you can implement on-demand pulling
             * After pulling the stream successfully, ZLMediaKit will immediately forward it to the player (the maximum waiting time is about 5 seconds, if it is not pulled successfully within 5 seconds, the player will play failed)
        ...
        // Listen to the playback or push stream end event to consume traffic
...
int main(int argc,char *argv[]) {
    ...
    // Load the configuration file, if the configuration file does not exist, create one
    loadIniConfig();
    initEventListener();

    // 这里是拉流地址，支持rtmp/rtsp协议，负载必须是H264+AAC  [AUTO-TRANSLATED:3f92b7aa]
    // This is the pull stream address, which supports rtmp/rtsp protocol, and the payload must be H264+AAC
    ...
    for (auto &url : urlList) {
        ...
        // Recording address (VLC does not support so many levels of RTMP URL, use test_player to test RTMP on-demand):
        ...
        // Start playing, if playback fails or is interrupted, it will automatically retry several times, the number of retries is configured in the configuration file, default is to retry indefinitely
        player->play(url);
        // 需要保存PlayerProxy，否则作用域结束就会销毁该对象  [AUTO-TRANSLATED:939a84c9]
        // Need to save PlayerProxy, otherwise the object will be destroyed when the scope ends
    ...
    // If SSL is supported, HTTPS server can also be enabled
    ...
    // RTSP server that supports SSL encryption, can be used for devices such as Amazon Echo Show
    TcpServer::Ptr rtspSSLSrv(new TcpServer());
    rtspSSLSrv->start<RtspSessionWithSSL>(rtspsPort);//默认322

    // 服务器支持动态切换端口(不影响现有连接)  [AUTO-TRANSLATED:28949b78]
    // Server supports dynamic port switching (does not affect existing connections)
