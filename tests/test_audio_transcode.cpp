...
struct TransCtx{
  ...
  TransCtx(const char* dir, CodecId codec) {
    char path[256];
    Track::Ptr track;
    switch (codec)
    {
    case CodecAAC:
      ...
      sprintf(path, "%s_aac.mp4", dir);
      break;
    case CodecOpus:
      track.reset(new OpusTrack());
      sprintf(path, "%s_opus.mp4", dir);
      break;
    case CodecG711A:
    case CodecG711U:
      track.reset(new G711Track(codec, 8000, 1, 16));
      sprintf(path, "%s_711%c.mp4", dir, codec == CodecG711A?'A':'U');
      break;
    default:
      return;
      break;
    ...
    enc->setOnEncode([this](const Frame::Ptr& frame){
      file->inputFrame(frame);
    });
    /*
