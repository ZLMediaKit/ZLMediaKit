...
int domain(const string &file, const string &url) {
    ...
    // Set the push stream interruption handling logic
    ...
    pusher->setOnShutdown([poller, url, weak_pusher, weak_src](const SockException &ex) {
        if (!weak_src.lock()) {
            ...
            // Media cancellation causes push stream interruption, no retry push
    ...
    // reader set to empty can terminate push-related resources
    // reader = nullptr;
