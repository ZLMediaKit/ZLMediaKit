...
class HttpWsClient : public HttpClientImp , public WebSocketSplitter{
    WebSocketHeader::Type _data_type = WebSocketHeader::TEXT;
public:
    WebSocketHeader::Type getDataType() const { return _data_type; }
    void setDataType(WebSocketHeader::Type type) {
        _data_type = type;
    }

    typedef std::shared_ptr<HttpWsClient> Ptr;

    HttpWsClient(const std::shared_ptr<SendInterceptor<ClientType> > &delegate)
        : _weak_delegate(delegate), _delegate(*delegate) {
    ...
    void onResponseHeader(const std::string &status, const HttpHeader &headers) override {
        if(status == "101") {
            ...
            if(Sec_WebSocket_Accept == const_cast<HttpHeader &>(headers)["Sec-WebSocket-Accept"]) {
                ...
                shutdown(toolkit::SockException(toolkit::Err_shutdown, "Sec-WebSocket-Accept mismatch"));
            }
        }
        else {
            shutdown(toolkit::SockException(toolkit::Err_shutdown, "bad http status code:" + status));
        }
    };
...
private:
    void onWebSocketException(const toolkit::SockException &ex){
        ...
        {
            //websocket握手成功
            //此处截取TcpClient派生类发送的数据, 并进行websocket协议打包
            ...
            _delegate.setOnBeforeSendCB([weakSelf](const toolkit::Buffer::Ptr &buf){
                if(auto strongSelf = weakSelf.lock()){
                    ...
                    header._fin = true;
                    header._reserved = 0;
                    WebSocketHeader::Type op = strongSelf->_data_type; // WebSocketHeader::TEXT
                    if (auto wsbuf = std::dynamic_pointer_cast<WebSocketBuffer>(buf))
                        op = wsbuf->headType();
                    header._opcode = op;
                    //客户端需要加密
                    header._mask_flag = true;
...
private:
    ...
    std::weak_ptr<SendInterceptor<ClientType>> _weak_delegate;
    SendInterceptor<ClientType> &_delegate;
    // websocket当前分片缓存
 ...
 */
template <typename ClientType, bool useWSS = false >
class WebSocketClient : public SendInterceptor<ClientType>{
public:
    typedef std::shared_ptr<WebSocketClient> Ptr;

    template<typename ...ArgsType>
    WebSocketClient(ArgsType &&...args) : SendInterceptor<ClientType>(std::forward<ArgsType>(args)...){
    }
    ...
    void startWebSocket(const std::string &ws_url, float fTimeOutSec = 3, WebSocketHeader::Type op = WebSocketHeader::TEXT) {
        _wsClient = std::make_shared<HttpWsClient<ClientType>>(std::static_pointer_cast<WebSocketClient>(this->shared_from_this()));
        _wsClient->setOnCreateSocket([this](const toolkit::EventPoller::Ptr &){
            return this->createSocket();
        });
        _wsClient->setDataType(op);
...
private:
    typename HttpWsClient<ClientType>::Ptr _wsClient;
};
