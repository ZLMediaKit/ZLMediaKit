...
public:
    ...
    // 开启或关闭录制
    virtual bool setupRecord(MediaSource &sender, Recorder::type type, bool start, const std::string &custom_path, size_t max_second, bool as_player) { return false; };
    // 获取录制状态
...
public:
    ...
    // 开启或关闭录制
    bool setupRecord(Recorder::type type, bool start, const std::string &custom_path, size_t max_second, bool as_player);
    // 获取录制状态
