...
void PlayerProxy::setPlayerArgs(const toolkit::mINI &args) {
    for (auto &key : _player_arg_keys) {
        erase(key);
    }
    _player_arg_keys.clear();
    _player_arg_keys.reserve(args.size());
    for (auto &pr : args) {
        (*this)[pr.first] = pr.second;
        _player_arg_keys.emplace_back(pr.first);
    }
    if (find(Client::kWaitTrackReady) == end()) {
        (*this)[Client::kWaitTrackReady] = false;
    }
}

void PlayerProxy::update(const std::string &url, const ProtocolOption &option, const toolkit::mINI &args) {
    ...
    auto is_connecting = _status && *_status == "connecting";
    _timer.reset();
    if (_live_status == 0 || is_connecting) {
        _pending_reload = true;
        _status = std::make_shared<std::string>("reloading");
        MediaPlayer::teardown();
        return;
    }

    restartUpdatedStream();
}
...
void PlayerProxy::play(const string &url) {
    ...
    auto piFailedCnt = std::make_shared<int>(0); // 连续播放失败次数
    setOnPlayResult([weakSelf, piFailedCnt](const SockException &err) {
        auto strongSelf = weakSelf.lock();
        if (!strongSelf) {
            return;
        }
        if (strongSelf->_pending_reload) {
            // Some players report onPlayResult() before the old transport is fully
            // shut down. Defer reload until onShutdown() to avoid overlapping the
            // new play() with the old delegate's teardown path.
            return;
        }
        if (err) {
    ...
    setOnShutdown([weakSelf, piFailedCnt](const SockException &err) {
        ...
        auto reload_requested = strongSelf->_pending_reload;
        if (!reload_requested && err) {
            NOTICE_EMIT(BroadcastPlayerProxyFailedArgs, Broadcast::kBroadcastPlayerProxyFailed, *strongSelf, err);
        }
        if (!reload_requested && strongSelf->_on_play) {
            strongSelf->_on_play(err);
            strongSelf->_on_play = nullptr;
        }
        if (reload_requested) {
            strongSelf->_status = std::make_shared<std::string>("reloading");
        } else {
        ...
        if (reload_requested) {
            strongSelf->_pending_reload = false;
            *piFailedCnt = 0;
            strongSelf->restartUpdatedStream();
            return;
        }
...
void PlayerProxy::restartUpdatedStream() {
    _live_status = 1;
    _timer.reset();
    try {
        _status = std::make_shared<std::string>("connecting");
