...
void RtspSession::handleReq_ANNOUNCE(const Parser &parser) {
    ...
    auto flag = NoticeCenter::Instance().emitEvent(
        Broadcast::kBroadcastMediaPublish, MediaOriginType::rtsp_push, _media_info, invoker, static_cast<SockInfo &>(*this));
    if (!flag) {
        //该事件无人监听,默认不鉴权
...
void RtspSession::emitOnPlay() {
    ...
    //广播通用播放url鉴权事件
    auto flag
        = _emit_on_play ? false : NoticeCenter::Instance().emitEvent(Broadcast::kBroadcastMediaPlayed, _media_info, invoker, static_cast<SockInfo &>(*this));
    if (!flag) {
        //该事件无人监听,默认不鉴权
        onRes("");
    }
        ...
        case Rtsp::RTP_MULTICAST: {
            if (!_multicaster) {
                _multicaster = RtpMultiCaster::get(
                    *this, get_local_ip(), _media_info.vhost, _media_info.app, _media_info.stream, _multicast_ip, _multicast_video_port, _multicast_audio_port);
                if (!_multicaster) {
                    send_NotAcceptable();
